...
class VDANN(nn.Module):
    def __init__(self,input_dim,z_dim=20,label_num=2,domain_num=11,beta=10,reverse=True):
        ...
        # z to mean
        self.encmean_fc11 = nn.Linear(int(self.input_dim/10/10/10*9), self.z_dim)
        # z to var
        self.encvar_fc12 = nn.Linear(int(self.input_dim/10/10/10*9), self.z_dim)
        # decoder
        #self.dec_fc1 = nn.Linear(z_dim, int(input_dim/10/10/10*9))
    ...
    def forward(self, x, alpha=0):
        # encode
        mu, logvar = self.encode(x.view(-1, 3, self.input_dim).float())
        # reparameterize
...
class VDANN_Trainer(Sound_Trainer):
    def __init__(self, z_dim = 20, label_num = 2, domain_num = 11, beta = 10, reverse=True, device="cpu"):
        ...
        # prepare dataset
        self.dataset = SoundDataset(transforms.ToTensor(), mode='DANN')
        # define model
        self.model = VDANN(input_dim=self.dataset.data_size, z_dim=z_dim, label_num=label_num, domain_num=domain_num, beta=beta).to(self.device)
        # define optimizer
    ...
    def export_latent_space(self, save_path='../result/VDANN/model/result.png'):
        # print z all data
...
def train_VDANN(key):
    vdann = VDANN_Trainer(beta=20)
    vdann.load(key)
    vdann.auto_train(1000, save_path = '../result/VDANN/' + key)
    vdann.export_latent_space(save_path = '../result/VDANN/' + key + '/z_map.png')
    vdann.save_weight(save_path = '../result/VDANN/' + key + '/vdann')
    del vdann
...
def Grad_CAM_VDANN(key):
    vdann = VDANN_Trainer(device='cpu')
    vdann.load_weight(load_path='../result/VDANN/'+key+'/vdann')
    vdann.load(key)
    vdann.model.eval()
    model_dict = dict(arch=vdann.model, layer_name=vdann.model.enc_conv3)
    Sound_Grad_CAM(vdann, model_dict, key, '../result/VDANN/'+key)
    Sound_Grad_CAM(vdann, model_dict, key, '../result/VDANN/'+key, pp_mode=False)
